...
- `neoforge/` – NeoForge platform glue: NeoForge entrypoints, NeoGradle run configs, mixin configs, and platform-specific implementations.
...
Why the change: The Create mod and some dependencies for 1.21.x require NeoForge. The project simplified from a multi-loader setup to a NeoForge-only layout to avoid mapping and compatibility conflicts.
 ...
 - Common tasks:
   ...
   - `gradlew :neoforge:runClient` — run NeoForge dev environment.
   - `gradlew :neoforge:runServer` — run NeoForge server dev environment.
   - `gradlew :neoforge:datagen` — run data generators (set DATAGEN=TRUE when required). Prefer NeoForge datagen or `common` data hooks.

IMPORTANT (porting): When working on the 1.21.* port, prefer these commands and focus on NeoForge runs. Do not assume Fabric is available in the port environment. Always verify builds on the NeoForge side and the `:common` module.

If you do need to run datagen, prefer the NeoForge datagen or the `common` hooks where available. If using the Fabric datagen run for reference, clearly document why it's required and mark it as temporary.
 ...
 - Always set DATAGEN=TRUE when running datagen locally (this repo checks this in code). Prefer the NeoForge datagen run or `common` generator hooks.

Emphasize build & error checks
- The port's priority is a clean NeoForge + common build on the 1.21.* line. Before suggesting changes or opening PRs, run the following checks and include results in PR descriptions:
  - `gradlew :common:build` and `gradlew :neoforge:build` (or `gradlew build` for a full run) to verify compilation and resource generation.
  - Use Gradle's `--scan` or `--stacktrace` flags when failures occur to capture detailed errors.
  - Run the repo error check tool (IDE problems view or `./gradlew :common:compileJava` / `:neoforge:compileJava`) and include the error output if proposing changes that affect build or runtime.
...
- Property access: Gradle scripts use a helper operator function operator fun String.invoke() to read values from `rootProject.ext`/`gradle.properties`. When suggesting changes to versions or build properties reference keys from `gradle.properties` (e.g. `neoforge_version`, `minecraft_version`).
 - Registrate & data-gen: The project uses Registrate heavily (see `common/registry/CRBlocks.java` and `CRItems.java`) and adds data generators in `Railways.gatherData`. When changing registration or data formats, update data generators in `common` and platform-specific generator adapters in `neoforge`.
 - Mixin/access-wideners: `common` contains `railways.accesswidener` (referenced by `common/build.gradle.kts`). `neoforge` configures access widener paths to reuse this file. Avoid duplicate AW edits; edit the single canonical file in `common/src/main/resources`.
...
- `common/src/main/java/com/railwayteam/railways/registry/CRBlocks.java` and `CRItems.java` — canonical registration and examples of Registrate usage and data-gen hooks.
 - `neoforge/build.gradle.kts` — platform run tasks and VM args (datagen, access widener wiring).
...
Integration points and external dependencies
 - Create mod (com.simibubi.create) is a required dependency; code assumes Create APIs and Registrate patterns. Be careful when importing Create classes into `common` — platform differences exist. Use platform adapters in `neoforge` where necessary.
...
Testing & debugging
 - Use the `:neoforge:runClient` task to test runtime behaviour in the NeoForge dev environment.
 - The project includes `run/options.txt` etc. The `run/` directory under `neoforge` may hold local run-state useful during debugging.
...
If you modify registration, datagen, or resource generation
 - Update the corresponding data generator (in `common`), and ensure platform adapters (under `neoforge`) either forward or implement platform-specific provider code.
...
- Use `CRBlocks.makeTrack(...)` patterns for creating block entries with Registrate + data-gen lambdas.
 - Use the `neoforge` datagen run config (in `neoforge/build.gradle.kts`) as canonical example for setting VM args and environment variable wiring when adding new runs.
...
When in doubt, prefer the `common/` implementation and update platform adapters only when the change touches platform APIs or requires platform-specific registries.
 ...
 If you need more context or a follow-up change (tests, build tweaks, or enabling a compat flag), ask which side (runtime or data-gen) and whether the change affects `common` or `neoforge`.
